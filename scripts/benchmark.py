...
if args.sanity:
    ...
    # fixed inputs
    gen_X, gen_y = torch.randn(B, 3, 32, 32).cuda(), torch.randint(
        10, [B], dtype=torch.long).cuda()
    ...
    # random inputs
    def gen_inputs(b):
        return torch.randn(b, 3, 32,
                           32).cuda(), torch.randint(10, [b],
                                                     dtype=torch.long).cuda()


def load_state_dict(src, tgt, B):
    w_optim = torch.optim.SGD(src.weights(),
                              0.1,
                              momentum=0.9,
                              weight_decay=0.003)
...
def benchmark(name, N, func, func_args, sanity=None):
    ...
    for i in range(N):
        ...
        if args.sanity and i == N - 1:
            ...
            if sanity:
                maxdiff = 0.
                for (fn, fw), sw in zip(sanity.named_parameters(),
                                        res.parameters()):
...
print("-" * 80)
standard = None
standard = benchmark("load_state_dict", N, load_state_dict,
                     [src, copy.deepcopy(tgt), B], standard)
